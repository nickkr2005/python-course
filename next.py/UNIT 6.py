...
# EX 6.1.1
# מפחיתים את גודל התוכנית בזיכרון
...
image_path = r"C:\Users\NickKR\OneDrive\Documents\fkying skoda.jpg"  # Replace with your image path
...
button = tk.Button(root, text="Whats my favorite video?", width=20, height=2, command=show_image)
...
# EX 6.2.2
# מבנה התנאי אחראי לכך שהפונקציה main בקובץ smile.py לא תרוץ כאשר מייבאים את הקובץ כמודול.


# EX 6.2.3
# כאשר נריץ את הקובץ orange.py


# EX 6.2.4
# הקובץ נמצא בנתיב הבא:
# C:\Heights\Documents\Projects\dandush.
...
class BirthdayCard(GreetingCard):
    ...
    def greeting_msg(self):
        ...
        print(f"Happy birthday {self._sender_age}")
...
# EX 6.3.2
# מייבאת את האובייקט c מתוך מודול b שנמצא בחבילה a.
...
line_color_first = (255, 0, 0)
line_color_second = (0, 0, 255)

def draw_lines(draw, points, line_color):
    for i in range(0, len(points)-2, 2):
        point1 = (points[i], points[i+1])
        point2 = (points[i+2], points[i+3])
        draw.line([point1, point2], fill=line_color, width=2)

draw_lines(draw, first, line_color_first)
draw_lines(draw, second, line_color_second)
image.save("connected_dots_image.png")
image.show()
